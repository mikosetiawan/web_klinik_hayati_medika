                                ...
                                <select class="form-control" id="id_dokter" name="id_dokter" required>
                                    ...
                                    @foreach ($dokters as $dokter)
                                        <option value="{{ $dokter->id }}">{{ $dokter->nama }} -
                                            {{ $dokter->spesialisasi }} ({{ $dokter->hari_mulai }} s/d
                                            {{ $dokter->hari_selesai }}, {{ $dokter->jam_mulai }} -
                                            {{ $dokter->jam_selesai }})</option>
                                    @endforeach
                                </select>
                                <div class="invalid-feedback">
                                    @error('id_dokter')
                ...
                <tbody>
                    @foreach ($pasiens as $pasien)
                        <tr>
                            ...
                            <td>{{ $pasien->dokter->hari_mulai }} s/d {{ $pasien->dokter->hari_selesai }},
                                {{ $pasien->dokter->jam_mulai }} - {{ $pasien->dokter->jam_selesai }}</td>
                            <td>{{ \Carbon\Carbon::parse($pasien->tanggal_pendaftaran)->format('d M Y') }}</td>
                            <td>
                                <span
                                    class="badge {{ $pasien->status == 'confirmed' ? 'bg-success' : ($pasien->status == 'cancelled' ? 'bg-danger' : ($pasien->status == 'finished' ? 'bg-info' : ($pasien->status == 'diagnosis' ? 'bg-primary' : ($pasien->status == 'paid' ? 'bg-success' : 'bg-warning')))) }}">
                                ...
                                <td>
                                    ...
                                    @if ($pasien->status == 'booked')
                                        <button class="btn btn-success btn-sm"
                                            onclick="updateStatus({{ $pasien->id }}, 'confirmed')">Konfirmasi</button>
                                        <button class="btn btn-warning btn-sm"
                                            onclick="updateStatus({{ $pasien->id }}, 'cancelled')">Batal</button>
                                        <button class="btn btn-danger btn-sm"
                                            onclick="deletePendaftaran({{ $pasien->id }})">Hapus</button>
                                    @elseif($pasien->status == 'confirmed')
                                        <button class="btn btn-primary btn-sm"
                                            onclick="updateStatus({{ $pasien->id }}, 'diagnosis')">Diagnosa</button>
                                    @elseif($pasien->status == 'diagnosis')
                                        <button class="btn btn-info btn-sm"
                                            onclick="updateStatus({{ $pasien->id }}, 'finished')">Selesai</button>
                        ...
                        <!-- Modal Detail -->
                        <div class="modal fadeქ
                            fade" id="detailModal{{ $pasien->id }}" tabindex="-1"
                            aria-labelledby="detailModalLabel{{ $pasien->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="detailModalLabel{{ $pasien->id }}">Detail
                                            Pendaftaran</h5>
                                    ...
                                    <div class="modal-body">
                                        <dl class="row">
                                            ...
                                            <dd class="col-sm-8">
                                                {{ \Carbon\Carbon::parse($pasien->pasien->tanggal_lahir)->format('d M Y') }}
                                            </dd>
                                            ...
                                            <dd class="col-sm-8">{{ $pasien->dokter->nama }}
                                                ({{ $pasien->dokter->spesialisasi }})</dd>
                                            <dt class="col-sm-4">Jadwal</dt>
                                            <dd class="col-sm-8">{{ $pasien->dokter->hari_mulai }} s/d
                                                {{ $pasien->dokter->hari_selesai }}, {{ $pasien->dokter->jam_mulai }}
                                                - {{ $pasien->dokter->jam_selesai }}</dd>
                                            ...
                                            <dd class="col-sm-8">
                                                @php
                                                    $hasilDiagnosa = App\Models\HasilDiagnosa::where(
                                                ...
                                                @endphp
                                                @if (!$hasilDiagnosa)
                                                    <p class="text-danger">Hasil diagnosa tidak ditemukan untuk
                                                        pendaftaran ini.</p>
                                                    ...
                                                    <ul class="list-group">
                                                        @foreach ($hasilDiagnosa->diagnosas as $diagnosa)
                                                            <li class="list-group-item">
                                                                ...
                                                                {{ $diagnosa->nama_diagnosa }} (Rp
                                                                {{ number_format($diagnosa->harga, 2, ',', '.') }})
                                                            </li>
                                                            @php
                                            ...
                                            <dd class="col-sm-8">
                                                @if (!$hasilDiagnosa)
                                                    <p class="text-danger">Hasil diagnosa tidak ditemukan, sehingga
                                                        data obat tidak tersedia.</p>
                                                ...
                                                @else
                                                    <ul class="list-group">
                                                        @foreach ($hasilDiagnosa->obats as $obat)
                                                            <li class="list-group-item">
                                                                {{ $obat->nama_obat }} (Dosis: {{ $obat->dosis }}) -
                                                                Rp {{ number_format($obat->harga, 2, ',', '.') }}
                                                            </li>
                                                            @php
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="button" class="btn btn-primary"
                                            onclick="downloadPDF({{ $pasien->id }})">Download PDF</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="modal fade" id="paymentModal{{ $pasien->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="paymentModalLabel{{ $pasien->id }}">Form
                                            Pembayaran</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form id="paymentForm{{ $pasien->id }}"
                                            action="{{ route('pembayaran.store') }}" method="POST"
                                            class="needs-validation" novalidate>
                                            @csrf
                                            <input type="hidden" name="pendaftaran_pasien_id"
                                                value="{{ $pasien->id }}">
                                            ...
                                            <!-- Total Harga -->
                                            <div class="mb-3">
                                                <label for="total_harga{{ $pasien->id }}" class="form-label">Total
                                                    Harga (Rp)</label>
                                                <input type="number" step="0.01"
                                                    class="form-control @error('total_harga') is-invalid @enderror"
                                                    id="total_harga{{ $pasien->id }}" name="total_harga" required
                                            ...
                                            <div class="mb-3">
                                                <label for="tanggal_pembayaran{{ $pasien->id }}"
                                                    class="form-label">Tanggal Pembayaran</label>
                                                <input type="date"
                                                    class="form-control @error('tanggal_pembayaran') is-invalid @enderror"
                                                    id="tanggal_pembayaran{{ $pasien->id }}"
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select
                                                    ...
                                                    <option value="bank_transfer"
                                                        {{ old('metode_pembayaran') == 'bank_transfer' ? 'selected' : '' }}>
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="submit" form="paymentForm{{ $pasien->id }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            console.log('Page loaded with user info:', {
                ...
                user_id: '{{ Auth::id() ?? 'no_id' }}',
                csrf_token: $('meta[name="csrf-token"]').attr('content')
            });

            // Remove the AJAX call for no_antrian since it's no longer needed in the form
            $('#id_dokter').on('change', function() {
                console.log('Doctor selected:', $(this).val());
                // No need to fetch no_antrian as it will be handled by the backend
            });
        ...
        function updateStatus(id, status) {
            const statusText = status === 'confirmed' ? 'Dikonfirmasi' : (status === 'diagnosis' ? 'Diagnosa' : (status ===
                'finished' ? 'Selesai' : (status === 'paid' ? 'Pembayaran' : 'Dibatalkan')));
            Swal.fire({
        ...
        function downloadPDF(id) {
            ...
            doc.text("Klinik Hayati Medika", 105, 15, {
                align: "center"
            });
            doc.setFontSize(12);
            doc.setFont("helvetica", "normal");
            doc.text("Cilegon - Indonesia", 105, 25, {
                align: "center"
            });
            ...
            if (diagnosaItems.length > 0) {
                doc.text(diagnosaText, 20, yPosition + 10, {
                    maxWidth: 160
                });
            ...
            if (obatItems.length > 0) {
                doc.text(obatText, 20, yPosition + 10, {
                    maxWidth: 160
                });
